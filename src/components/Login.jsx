    ...
    if (!isSignInForm) {
      ...
      createUserWithEmailAndPassword(auth, email.current.value, password.current.value)
        .then((userCredential) => {
          ...
          updateProfile(user, {
            displayName: name.current.value, photoURL: "https://wallpapers.com/images/high/pikachu-face-illustration-x8q0l4i7d7osqs2h.png"
          }).then(() => {
            // Profile updated!
    ...
    <div className="relative h-screen w-full">
      ...
      {isSignInForm ? (<div className="flex items-center justify-center h-screen">
        <form className="absolute scale-110 w-96 p-10 bg-black/50 text-white rounded-lg">
          ...
          <button className="w-full bg-red-600 hover:bg-red-700 p-2 rounded text-white font-semibold transition duration-300 cursor-pointer" onClick={(e) => { e.preventDefault(); handleButtonClick(); }}>
            Sign In
          </button>
        ...
        <form className="absolute scale-110 w-100 p-10 bg-neutral-800/10 backdrop-blur-lg text-white rounded-lg">
          ...
          <button className="w-full bg-red-600 hover:bg-red-700 p-2 text-white font-semibold transition duration-300 cursor-pointer rounded-xl my-2" onClick={(e) => { e.preventDefault(); handleButtonClick(); }}>
            Get Started
          </button>
